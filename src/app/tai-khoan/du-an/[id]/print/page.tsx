...
export default async function CustomerPrintProjectQuotationPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // --- IDOR Protection ---
  // If the user is logged in as customer (and not admin), they can only view their own projects
  if (!adminSession && customerSession) {
    if (project.customerId !== customerSession.customerId) {
      notFound(); // Prevent leaking project information
    }
  }
